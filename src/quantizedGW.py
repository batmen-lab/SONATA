"""
Code from: QUANTIZED GROMOV-WASSERSTEIN
https://github.com/trneedham/QuantizedGromovWasserstein/tree/main
https://arxiv.org/pdf/2104.02013
"""
import numpy as np
...
def find_submatching(pm,pn):
    ...
    coup_sub, log = ot.gromov.gromov_wasserstein(Distm, Distn, pm, pn)
...
def compressed_gw_ts(dists1,dists2,p1,p2,node_subset1,node_subset2,
    ...
    if verbose:
        print('Matching Compressed Graphs...')
    coup_compressed, log = ot.gromov.gromov_wasserstein(Dist_new1, Dist_new2, np.array(p_new1).reshape(-1), np.array(p_new2).reshape(-1))
    if verbose:
        print('Time for Matching Compressed:', time.time() - start)
...
def get_compressed_coupling_ts(dists1,dists2,p1,p2,node_subset1,node_subset2):
    ...
    Dist_new1, p_new1,coup1 = compress_graph_from_subset_ts(dists1,p1,node_subset1)
    Dist_new2, p_new2,coup2 = compress_graph_from_subset_ts(dists2,p2,node_subset2)
    coup_compressed, log = ot.gromov.gromov_wasserstein(Dist_new1, Dist_new2,
